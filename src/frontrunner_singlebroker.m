...
>                [(exidA, buysizeA), (exidB, buysizeB)] = decideorders (tracehd "broker:abooks1" abooks1) (tracehd "broker:abooks2" abooks2) 0 0 broker_mo_size
...
>                                                                      (dotrade=True) &
>                                                                      (oldprice=0)                                                           
>                                     = [NoOrderTuple id (newinv,newcash),Ordertuple id 1 (bid, ask, buy, sell, (newinv,newcash))], if (oldprice~=0)
>                                     = [NoOrderTuple id (newinv,newcash), NoOrderTuple id (newinv,newcash)], otherwise
...
>                             = exch2size, if(oldsize=0) & (isvalidordertuple (order_for_timestep!1))
>                             = oldsize, otherwise
>                    bid  = NoOrder 
>                    buy  = Order Buy exch2size 0       id time 1, if (oldprice=0)
>                         = NoOrder, otherwise
>                    ask  = Order Ask oldsize oldprice id time 1, if (oldprice~=0)
>                         = NoOrder, otherwise
>                    sell = NoOrder 
>                    (dotrade,atprice) = newbestask (index 2 abooks2) exch2size
>                                        where
...
>agents agentinput = [
>                     (brokerexch1delay, brokerexch2delay,(broker      initxq agentinput (AgentState 0 0 (0,0))  buyer_start_time)),     || NOTE all agents must have the same type - eg the same number of args 
>                     (popexch1delay, popexch2delay,      (populator   initxq agentinput (AgentState 0 0 (0,0)) (buyer_start_time-1))),  || NOTE all agents must have the same type - eg the same number of args 
>                     (frontexch1delay, frontexch2delay,  (frontrunner initxq agentinput (AgentState 0 0 (0,0))  buyer_start_time))      || NOTE all agents must have the same type - eg the same number of args 
>                    ] 
>                    where 
>                    initxq = ([],[],[],[])
>                    (brokerexch1delay, brokerexch2delay) = (0,5)
...
>populator_sizes = [1500,800,750,900,250,600,1720,500,300,175]
>populator_prices = [53,57,62,68,71,75,52,65,54,66]
>broker_mo_size = 7000
>debug_asks = True
